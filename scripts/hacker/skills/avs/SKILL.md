...
1. `avs snapshot`
2. 读输出，找到目标控件的编号
3. `avs tap <id>`（或 `avs tap --text 允许`）
4. **再次 `avs snapshot`** 确认结果 —— 每次 tap/key 之后旧编号立即作废；前台应用变化或 snapshot 超过 10 分钟也会作废，必须重新 snapshot
...
- `[n]` 是编号，`tap` 用它；`@(x,y)` 是设备像素中心点（左上角为原点）
- `source=tree` 来自界面树，`vision` 来自视觉模型，`hybrid` 两者合并
- 行尾可能有 `checked` / `unchecked` / `disabled`；无文字的控件显示资源 id，如 `[5] button #fab_add`
